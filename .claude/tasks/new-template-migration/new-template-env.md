...
Значения ниже уже прописаны в `scripts/lib/new-template-env.sh`, который сорсится
**только** из `replit-dev-backend.sh` — то есть их получает бэкенд воркспейса, и никто больше.
Опубликованное приложение поднимается через `replit-run.sh`, тот читает лишь
`lib/deploy-env.sh`, поэтому прод остаётся на старом шаблоне из application.yml.
Секреты выставлять не нужно; выставленный секрет всё равно победит (`${VAR:-…}`).

Чтобы раскатать новый шаблон на прод: перенести значения в дефолты application.yml
и удалить этот файл вместе со строкой его подключения.

## env-блок (справочно — уже прописан в скрипте)
