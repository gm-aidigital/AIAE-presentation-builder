...
public class TacticResolvers {
	...
	private final PacingCalculator pacing;
	...
	public TacticResolvers(
			...
			TacticExtractionHelper tacticExtraction, CampaignResolvers campaignResolvers, PacingCalculator pacing) {
	...
	/**
	 * Looks up tactic {@code n}'s metrics re-aggregated over the EOM reporting period, i.e. the actuals
	 * the "plan ctd" / "proj" / "vs goal" resolvers compare against the prorated goal.
	 *
	 * @param data campaign data whose {@code periodTactics()} may carry the period-scoped aggregation
	 * @param n    one-based tactic index
	 * @return the tactic's period-scoped metrics, or {@code null} when no reporting period was set or
	 * the tactic has no period data
	 */
	Tactic periodTactic(CampaignData data, int n) {

		Map<Integer, Tactic> periodTactics = data == null ? null : data.periodTactics();
		return periodTactics == null ? null : periodTactics.get(n);
	}

	/**
	 * Resolves the prorated to-date impressions goal for tactic {@code n}: the full-campaign planned
	 * impressions scaled by periodDays / flightDays, preferring a manual override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N imps plan ctd:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the full-campaign plan and the reporting-period/flight window
	 * @return the resolved prorated goal with its source tag, or a {@code not_found} placeholder when no
	 * reporting period or planned impressions figure is available
	 */
	public Resolved resolveTacticImpsPlanCtd(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		long[] days = pacing.periodAndFlightDays(data);
		Tactic t = tactic(data, n);
		Double planImps = t == null ? null : t.planImps();
		if (days == null || planImps == null) {
			return new Resolved(label, null, "not_found");
		}
		double planCtd = pacing.planCtd(planImps, days[0], days[1]);
		return new Resolved(label + " (auto: plan × period/flight days)", fmt.intGroup(planCtd), "adj");
	}

	/**
	 * Resolves the pace-based end-of-campaign impressions projection for tactic {@code n}: the
	 * period's actual impressions run-rate extrapolated across the whole flight, preferring a manual
	 * override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N imps proj:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the period actuals and the reporting-period/flight window
	 * @return the resolved projection with its source tag, or a {@code not_found} placeholder when no
	 * reporting period or period-actual impressions figure is available
	 */
	public Resolved resolveTacticImpsProj(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		long[] days = pacing.periodAndFlightDays(data);
		Tactic periodT = periodTactic(data, n);
		if (days == null || periodT == null) {
			return new Resolved(label, null, "not_found");
		}
		double proj = pacing.projection(periodT.imps(), days[0], days[1]);
		return new Resolved(label + " (auto: period run-rate × flight days)", fmt.intGroup(proj), "adj");
	}

	/**
	 * Resolves the impressions variance of tactic {@code n}'s period actual against its prorated to-date
	 ...
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the plan, the period actual and the reporting-period/flight window
	...
	public Resolved resolveTacticImpsVsGoal(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		Tactic periodT = periodTactic(data, n);
		Double planImps = t == null ? null : t.planImps();
		if (days == null || periodT == null || planImps == null) {
			return new Resolved(label, null, "not_found");
		}
		double planCtd = pacing.planCtd(planImps, days[0], days[1]);
		String variance = pacing.paceVariance(periodT.imps(), planCtd, false);
		if (variance == null) {
			return new Resolved(label, null, "not_found");
		}
		return new Resolved(label + " (auto: period actual vs prorated goal)", variance, "adj");
	}
	 ...
	 * same as the full-campaign planned CTR, preferring a manual override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N ctr plan ctd:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the full-campaign planned CTR
	...
	public Resolved resolveTacticCtrPlanCtd(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		return new Resolved(label + " (auto: Estimates CTR, rate target unaffected by period)",
				fmt.dec2(planCtr) + "%", "adj");
	}

	/**
	 * Resolves the projected end-of-campaign CTR for tactic {@code n}. A rate metric's projection is
	 * simply the period's own observed rate carried forward (rates don't accumulate the way volumes
	 * do), preferring a manual override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N ctr proj:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the period actual CTR
	 * @return the resolved projection with its source tag, or a {@code not_found} placeholder when no
	 * period actual CTR is available
	 */
	public Resolved resolveTacticCtrProj(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		Tactic periodT = periodTactic(data, n);
		Double ctr = periodT == null ? null : periodT.ctr();
		if (ctr == null) {
			return new Resolved(label, null, "not_found");
		}
		return new Resolved(label + " (auto: period actual rate held flat)", fmt.dec2(ctr) + "%", "adj");
	}

	/**
	 * Resolves the CTR variance of tactic {@code n}'s period actual against its planned CTR (in
	 ...
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the planned CTR and the period actual CTR
	 * @return the resolved variance with its source tag, or a {@code not_found} placeholder when the
	 * goal or actual can't be computed
	 */
	public Resolved resolveTacticCtrVsGoal(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		return new Resolved(label + " (auto: period actual vs planned rate)", variance, "adj");
	}

	/**
	 * Resolves the prorated to-date VCR goal for tactic {@code n}; a rate target, unaffected by the
	 * reporting period, preferring a manual override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N vcr plan ctd:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the full-campaign planned VCR
	 * @return the resolved goal with its source tag, or a {@code not_found} placeholder when no planned
	 * VCR is available
	 */
	public Resolved resolveTacticVcrPlanCtd(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		return new Resolved(label + " (auto: Estimates VCR, rate target unaffected by period)",
				Math.round(planVcr) + "%", "adj");
	}

	/**
	 * Resolves the projected end-of-campaign VCR for tactic {@code n}, carrying the period's observed
	 * rate forward, preferring a manual override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N vcr proj:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the period actual VCR
	 * @return the resolved projection with its source tag, or a {@code not_found} placeholder when no
	 * period actual VCR is available
	 */
	public Resolved resolveTacticVcrProj(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		Tactic periodT = periodTactic(data, n);
		Double vcr = periodT == null ? null : periodT.vcr();
		if (vcr == null) {
			return new Resolved(label, null, "not_found");
		}
		return new Resolved(label + " (auto: period actual rate held flat)", Math.round(vcr) + "%", "adj");
	}

	/**
	 * Resolves the VCR variance of tactic {@code n}'s period actual against its planned VCR (in
	 * percentage points), preferring a manual override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N vcr vs goal:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the planned VCR and the period actual VCR
	 * @return the resolved variance with its source tag, or a {@code not_found} placeholder when the
	 * goal or actual can't be computed
	 */
	 ...
	 * periodDays / flightDays, preferring a manual override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N completions plan ctd:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the planned impressions/VCR and the reporting-period/flight window
	...
	public Resolved resolveTacticCompletionsPlanCtd(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		if (days == null || planCompletions == null) {
			return new Resolved(label, null, "not_found");
		}
		double planCtd = pacing.planCtd(planCompletions, days[0], days[1]);
		return new Resolved(label + " (auto: planned imps × VCR, prorated)", fmt.intGroup(planCtd), "adj");
	}

	/**
	 * Resolves the pace-based end-of-campaign completions projection for tactic {@code n}, preferring a
	 * manual override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N completions proj:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the period actual completions and the reporting-period/flight window
	...
	public Resolved resolveTacticCompletionsProj(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		double proj = pacing.projection(periodT.completions(), days[0], days[1]);
		return new Resolved(label + " (auto: period run-rate × flight days)", fmt.intGroup(proj), "adj");
	}

	/**
	 * Resolves the completions variance of tactic {@code n}'s period actual against its prorated
	 ...
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the plan inputs, the period actual and the reporting-period/flight window
	 * @return the resolved variance with its source tag, or a {@code not_found} placeholder when the
	 * goal or actual can't be computed
	 */
	public Resolved resolveTacticCompletionsVsGoal(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		Tactic periodT = periodTactic(data, n);
		Double planCompletions = planCompletions(data, n);
		if (days == null || periodT == null || planCompletions == null) {
			return new Resolved(label, null, "not_found");
		}
		double planCtd = pacing.planCtd(planCompletions, days[0], days[1]);
		String variance = pacing.paceVariance(periodT.completions(), planCtd, false);
		if (variance == null) {
			return new Resolved(label, null, "not_found");
		}
		return new Resolved(label + " (auto: period actual vs prorated goal)", variance, "adj");
	 ...
	 * by periodDays / flightDays, preferring a manual override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N reach plan ctd:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the planned impressions/frequency and the reporting-period/flight window
	 * @return the resolved prorated goal with its source tag, or a {@code not_found} placeholder when the
	 * inputs are unavailable
	 */
	public Resolved resolveTacticReachPlanCtd(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		if (days == null || planReach == null) {
			return new Resolved(label, null, "not_found");
		}
		double planCtd = pacing.planCtd(planReach, days[0], days[1]);
		return new Resolved(label + " (auto: planned imps ÷ freq, prorated)", fmt.intGroup(planCtd), "adj");
	}

	/**
	 * Resolves the pace-based end-of-campaign reach projection for tactic {@code n}, using the same
	 * imps ÷ freq derivation as the actual reach figure for the period, preferring a manual override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N reach proj:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the period actual impressions/frequency and the reporting-period/flight window
	 * @return the resolved projection with its source tag, or a {@code not_found} placeholder when
	 * unavailable
	 */
	public Resolved resolveTacticReachProj(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		long[] days = pacing.periodAndFlightDays(data);
		Double periodReach = periodReach(data, n);
		if (days == null || periodReach == null) {
			return new Resolved(label, null, "not_found");
		}
		double proj = pacing.projection(periodReach, days[0], days[1]);
		return new Resolved(label + " (auto: period run-rate × flight days)", fmt.intGroup(proj), "adj");
	}

	/**
	 * Resolves the reach variance of tactic {@code n}'s period actual against its prorated to-date
	 * goal, preferring a manual override.
	 *
	...
	public Resolved resolveTacticReachVsGoal(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		Double planReach = planReach(data, n);
		Double periodReach = periodReach(data, n);
		if (days == null || planReach == null || periodReach == null) {
			return new Resolved(label, null, "not_found");
		}
		double planCtd = pacing.planCtd(planReach, days[0], days[1]);
		String variance = pacing.paceVariance(periodReach, planCtd, false);
		if (variance == null) {
			return new Resolved(label, null, "not_found");
		}
		return new Resolved(label + " (auto: period actual vs prorated goal)", variance, "adj");
	 ...
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the full-campaign planned spend and the reporting-period/flight window
	...
	public Resolved resolveTacticSpendPlanCtd(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		Tactic t = tactic(data, n);
		Double planSpend = t == null ? null : t.planSpend();
		if (days == null || planSpend == null) {
			return new Resolved(label, null, "not_found");
		}
		double planCtd = pacing.planCtd(planSpend, days[0], days[1]);
		return new Resolved(label + " (auto: plan × period/flight days)", fmt.moneyExact(planCtd), "adj");
	}

	/**
	 * Resolves the spend pacing percentage of tactic {@code n}'s period actual against its prorated
	 ...
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the plan inputs, the period actual spend and the reporting-period/flight window
	...
	public Resolved resolveTacticSpendPace(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		Tactic periodT = periodTactic(data, n);
		Double planSpend = t == null ? null : t.planSpend();
		if (days == null || periodT == null || planSpend == null) {
			return new Resolved(label, null, "not_found");
		}
		double planCtd = pacing.planCtd(planSpend, days[0], days[1]);
		String variance = pacing.paceVariance(periodT.spend(), planCtd, false);
		if (variance == null) {
			return new Resolved(label, null, "not_found");
		}
		return new Resolved(label + " (auto: period actual vs prorated goal)", variance, "adj");
	...
	/**
	 * Resolves the actual cost-per-thousand-impressions for tactic {@code n} over the reporting
	 * period, a metric that never existed in EOC, preferring a manual override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N cpm:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the period actual spend/impressions
	 * @return the resolved CPM with its source tag, or a {@code not_found} placeholder when period
	...
	public Resolved resolveTacticCpm(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		Tactic periodT = periodTactic(data, n);
		if (periodT == null || periodT.imps() <= 0) {
			return new Resolved(label, null, "not_found");
		}
		double cpm = periodT.spend() / periodT.imps() * 1000;
		return new Resolved(label + " (auto: period spend / imps × 1000)", fmt.moneyExact(cpm), "adj");
	}

	/**
	 * Resolves the prorated to-date CPM goal for tactic {@code n}: the full-campaign planned
	 * spend/impressions ratio, a cost-efficiency target unaffected by the reporting period, preferring
	 * a manual override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N cpm plan ctd:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the full-campaign planned spend/impressions
	...
	public Resolved resolveTacticCpmPlanCtd(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		return new Resolved(label + " (auto: planned spend / imps × 1000, rate target unaffected by period)",
				fmt.moneyExact(planCpm), "adj");
	}

	/**
	 * Resolves the projected end-of-campaign CPM for tactic {@code n}, carrying the period's observed
	 * cost efficiency forward, preferring a manual override.
	 *
	 ...
	 * @param data      campaign data providing the period actual spend/impressions
	 * @return the resolved projection with its source tag, or a {@code not_found} placeholder when
	 * period actuals are unavailable
	 */
	public Resolved resolveTacticCpmProj(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		double cpm = periodT.spend() / periodT.imps() * 1000;
		return new Resolved(label + " (auto: period actual rate held flat)", fmt.moneyExact(cpm), "adj");
	}

	/**
	 * Resolves the CPM variance of tactic {@code n}'s period actual against its planned CPM, preferring
	 * a manual override.
	 *
	 * @param n         one-based tactic index used to build the {@code "Tactic N cpm vs goal:"} lookup label
	 * @param sheetRows Media Plan grid rows searched for the labelled value
	 * @param adjRows   manual Adjustments grid rows that take precedence over the sheet
	 * @param data      campaign data providing the planned and period actual spend/impressions
	 * @return the resolved variance with its source tag, or a {@code not_found} placeholder when the
	 * goal or actual can't be computed
	 */
	public Resolved resolveTacticCpmVsGoal(int n, List<List<String>> sheetRows, List<List<String>> adjRows,
		...
		Double planCpm = planCpm(data, n);
		Tactic periodT = periodTactic(data, n);
		if (planCpm == null || periodT == null || periodT.imps() <= 0) {
			return new Resolved(label, null, "not_found");
		}
		double cpm = periodT.spend() / periodT.imps() * 1000;
	...
	/**
	 * Derives tactic {@code n}'s period-actual reach as period impressions ÷ the same derived
	 * frequency {@link #resolveTacticFreq} uses for the full-flight actual figure (frequency is not
	 * separately measured per period in the current data model).
	 *
	 * @param data campaign data supplying the period impressions and the planned max frequency
	 * @param n    one-based tactic index
	 * @return the derived period reach, or {@code null} when the inputs are missing or non-positive
	 */
	Double periodReach(CampaignData data, int n) {

		Tactic t = tactic(data, n);
		Tactic periodT = periodTactic(data, n);
		if (t == null || periodT == null || t.planMaxFreq() == null || t.planMaxFreq() <= 0 || periodT.imps() <= 0) {
			return null;
		}
		double freq = tacticExtraction.freqFromMax(n, t.planMaxFreq());
		return freq > 0 ? periodT.imps() / freq : null;
	}
