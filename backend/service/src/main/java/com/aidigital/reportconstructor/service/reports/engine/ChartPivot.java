	...
	public Pivot buildDailyPivot(
		...
		if (rows != null) {
			for (List<String> row : rows) {
				...
				double clicks = h.clicksCol() >= 0 ? reportNumbers.parseReportNumber(sheetRows.cellAt(row,
						h.clicksCol())) : 0.0;
				double completions = h.completionsCol() >= 0 ? reportNumbers.parseReportNumber(sheetRows.cellAt(row,
						h.completionsCol())) : 0.0;
