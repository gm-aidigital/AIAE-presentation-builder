...
public class ClaudeBatchPromptBuilder {
     ...
     *
     * @param data  parsed campaign plan and per-tactic performance used to assemble the brief/plan/tactic context blocks
     * @param brief free-text campaign brief prepended as the {@code === CAMPAIGN BRIEF ===} section (treated as empty when null)
     * @return the Batch A strategic prompt requesting audience/proposal/insights JSON, or empty when no context block could be built
     */
     ...
     *
     * @param data  parsed campaign data; its tactic map drives the per-tactic gender/peak-time estimation and the required JSON keys
     * @param brief free-text campaign brief prepended as the {@code === CAMPAIGN BRIEF ===} section (treated as empty when null)
     * @return the Batch B prompt requesting per-tactic gender split and weekday/weekend peak windows, or empty when no tactics exist
     */
     ...
     *
     * @param data  parsed campaign data including overall totals and actual-vs-plan tactic metrics used for the results context blocks
     * @param brief free-text campaign brief prepended as the {@code === CAMPAIGN BRIEF ===} section (treated as empty when null)
     * @return the Batch C post-campaign prompt requesting results overview, performance thoughts and per-tactic overviews JSON, or empty when no context block could be built
     */
     ...
     *
     * @param geoRows rows of the media-plan "Geo" tab; each inner list is one row whose cells are joined with {@code " | "} (null rows are skipped)
     * @return a prompt asking Claude to condense the listed locations into a single short comma-separated string of key regions
     */
